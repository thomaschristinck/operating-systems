...
Store store[POD_NUMBERS] [POD_LENGTH];
...
int kv_store_write(char* name, int key, char* value)
{
	...
	store[0][0].addr = mmap(NULL, VALUE_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
	for(int i = 1; i < 16; i++)
	{
	...
	ftruncate(fd, POD_LENGTH * POD_NUMBERS);
	close(fd);
