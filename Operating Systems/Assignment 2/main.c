...
int main(void)
{
    ...
    while (1)
    {
        ...
        char* str = "Greetings from shared memory!";
        char* oldkey = "McGill";
        kv_store_write(oldkey, str);
        char* print = kv_store_read(oldkey);
        printf("Key:%s, Args 0: %s\n", oldkey, print);

         char* newstr = "Hello how are you bud?";
        char* newkey = "Montreal";
        kv_store_write(newkey, newstr);
        char* newprint = kv_store_read(newkey);
        printf("Key:%s, Value read: %s\n", newkey, newprint);

        kv_store_write(oldkey, newstr);
        char* lastprint = kv_store_read(oldkey);
        printf("Key:%s, Value read: %s\n", oldkey, lastprint);
